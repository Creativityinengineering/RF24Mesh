 ...
 */
 ...
 template<class network_t, class radio_t>
 uint8_t ESBMesh<network_t, radio_t>::updateAddress(uint8_t nodeID, uint16_t address) //Falls der Master die bereits angemeldeten Slaves noch nicht kennt, diese aber hinzugefuegt werden sollen.
 {
     ...
     // Prüfen, ob MeshAdresse bereits vorhanden sind
     for (uint8_t i = 0; i < addrListTop; i++) {
         if (addrList[i].address == address) {
             if(addrList[i].nodeID != nodeID)
             return 2; // andere Node ID hat bereits die selbe Adresse ==> neue Addresse senden triggern
         }
     }
